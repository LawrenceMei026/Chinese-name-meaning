...
const rejectedCandidates = await readFile(rejectedUrl, 'utf8')
  ...
  .catch(error => {
    ...
    return {
      source: queue.source,
      reviewPolicy: {
        status: 'script_rejected_terminal',
        transformation: 'Candidates rejected by the automatic review policy are retained as terminal review records and are not applied to production dictionary data.',
      },
      entries: {},
    }
  })
...
if (
  rejectedCandidates.source.name !== queue.source.name
  || rejectedCandidates.source.dumpDate !== queue.source.dumpDate
  || rejectedCandidates.source.file !== queue.source.file
  || rejectedCandidates.source.sha1 !== queue.source.sha1
  || rejectedCandidates.source.license !== queue.source.license
  || rejectedCandidates.reviewPolicy?.status !== 'script_rejected_terminal'
) throw new Error('Chinese Wiktionary rejected candidate records are missing fixed source metadata.')

let approved = 0
let skippedExisting = 0
let skippedUsableDefinition = 0
let terminalRejected = 0
let skippedExistingRejected = 0
for (const [char, review] of Object.entries(autoReview.entries)) {
  if (review.decision === 'reject') {
    if (rejectedCandidates.entries[char]) {
      skippedExistingRejected += 1
      continue
    }
    const candidate = queue.entries[char]
    if (!candidate) throw new Error(`${char}: rejected source candidate is missing from the review queue`)
    if (
      review.revisionId !== candidate.revisionId
      || review.timestamp !== candidate.timestamp
      || review.sourceUrl !== candidate.sourceUrl
    ) throw new Error(`${char}: rejected review source revision does not match the fixed queue`)
    rejectedCandidates.entries[char] = {
      revisionId: candidate.revisionId,
      timestamp: candidate.timestamp,
      sourceUrl: candidate.sourceUrl,
      rejectedDefinitions: review.rejectedDefinitions,
      reasons: review.reasons,
      reviewed: true,
      reviewMethod: 'script_rejected_terminal',
    }
    terminalRejected += 1
    continue
  }
  if (review.decision !== 'manual_review') continue
  ...
  supplements.entries[char] = {
    ...
    reviewed: true,
    reviewMethod: 'script_approved_pending_review',
  }
  approved += 1
}

await writeFile(supplementsUrl, `${JSON.stringify(supplements, null, 2)}\n`)
await writeFile(rejectedUrl, `${JSON.stringify(rejectedCandidates, null, 2)}\n`)
console.log(`Script-approved ${approved} pending Chinese Wiktionary supplements; terminal-rejected ${terminalRejected} candidates; skipped ${skippedExisting} existing entries, ${skippedUsableDefinition} dictionary entries with usable definitions, and ${skippedExistingRejected} existing rejected records.`)
