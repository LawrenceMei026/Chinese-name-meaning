...
const supplementsUrl = new URL('../src/data/xinhuaDefinitionSupplements.json', import.meta.url)
...
if (
  supplements.source.name !== 'Xinhua dictionary structured data'
  || !/^[a-f0-9]{64}$/u.test(supplements.source.sha256)
  || supplements.corroborationSource.name !== 'Make Me a Hanzi dictionary.txt'
  || supplements.corroborationSource.license !== 'LGPL-3.0-or-later'
  || !/^[a-f0-9]{64}$/u.test(supplements.corroborationSource.sha256)
) throw new Error('Xinhua supplement source metadata is incomplete.')
...
for (const [char, supplement] of Object.entries(supplements.entries)) {
  ...
  if (supplement.reviewed !== true) throw new Error(`${char}: Xinhua supplement has not been reviewed`)
  if (!supplement.selectedText.includes(supplement.sourceWord)) throw new Error(`${char}: selected text does not match source word`)
...
if (checkOnly) {
  if (changed > 0 || generated !== dictionaryText) throw new Error(`Xinhua supplements are not applied (${changed} definitions differ).`)
  console.log(`Verified ${Object.keys(supplements.entries).length} reviewed Xinhua supplements.`)
} else {
  await writeFile(dictionaryUrl, generated)
  console.log(`Applied ${changed} of ${Object.keys(supplements.entries).length} reviewed Xinhua supplements.`)
}
