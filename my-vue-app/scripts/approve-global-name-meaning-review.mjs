...
const decisions = await readFile(decisionsUrl, 'utf8')
  .then(text => JSON.parse(text))
  .catch(error => {
    if (error.code !== 'ENOENT') throw error
    return {
      source: {
        name: 'Local dictionary coverage audit',
        dictionaryFile: 'public/data/chars.json',
        culturalFile: 'src/data/cultural.json',
        transformation: 'Characters with no naming-usable dictionary definition or cultural meaning are terminally reviewed without fabricating fallback meanings.',
      },
      entries: {},
    }
  })
...
for (const [char, entry] of reachable) {
  const culture = cultural[char]
  const definition = hasMeaningfulDefinition(entry.definition_cn) ? entry.definition_cn : ''
  if (hasUsableCulturalData(culture) || cleanDefinition(definition)) {
    if (decisions.entries[char]) {
      delete decisions.entries[char]
      removedResolved += 1
    }
    continue
  }
  if (decisions.entries[char]?.reviewed === true) {
    alreadyReviewed += 1
    continue
  }
  decisions.entries[char] = {
    decision: 'terminal_no_usable_definition_source',
    reviewed: true,
    reviewMethod: 'script_approved_global_pending',
    pinyin: entry.pinyin ?? '',
    tones: entry.tones ?? '',
    radical: entry.radical ?? '',
    originalDefinitionCn: entry.definition_cn ?? '',
    reason: 'No naming-usable modern definition or cultural meaning is available from the current local data sources; no fallback meaning was fabricated.',
  }
  reviewed += 1
}

await writeFile(decisionsUrl, `${JSON.stringify(decisions, null, 2)}\n`)
console.log(`Script-reviewed ${reviewed} global pending characters; ${alreadyReviewed} were already terminally reviewed; removed ${removedResolved} resolved terminal records.`)
